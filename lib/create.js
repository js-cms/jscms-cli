...
module.exports = async function create(folderName, options) {
  ...
  fs.copySync(__dirname + '/data/extra/resource', path.join(projectFolderPath, 'app/static/upload'));
  log('processing', `The resource file is copied.`);
  ...
  fs.outputFileSync(path.join(projectFolderPath, `ssl/1_www.${domain}_bundle.crt`), '');
  fs.outputFileSync(path.join(projectFolderPath, `ssl/2_www.${domain}.key`), '');
  ...
  console.log('shell run: npm install && npm start\n');
  process.exit();
}
